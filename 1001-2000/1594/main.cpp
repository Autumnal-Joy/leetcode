...
public:
    int maxProductPath(const vector<vector<int>> &grid) {
        ...
        auto min_dp = vector(m, vector(n, 0ll));
        auto max_dp = vector(m, vector(n, 0ll));
        min_dp[0][0] = max_dp[0][0] = grid[0][0];
        for (int i = 1; i < m; ++i) {
            min_dp[i][0] = max_dp[i][0] = min_dp[i - 1][0] * grid[i][0];
        }
        for (int j = 1; j < n; ++j) {
            min_dp[0][j] = max_dp[0][j] = min_dp[0][j - 1] * grid[0][j];
        }
        for (int i = 1; i < m; ++i) {
            for (int j = 1; j < n; ++j) {
                max_dp[i][j] = std::max(
        ...
        return max_dp[m - 1][n - 1] < 0 ? -1 : static_cast<int>(max_dp[m - 1][n - 1] % M);
    }
};
