...
public:
    vector<int> secondGreaterElement(const vector<int> &nums) {
        ...
        auto res = vector(n, -1);
        auto stk1 = vector<int>();
        auto stk2 = vector<int>();
        for (int i = 0; i < n; ++i) {
            while (!stk2.empty() && nums[i] > nums[stk2.back()]) {
                res[stk2.back()] = nums[i];
                stk2.pop_back();
            }
            auto j = static_cast<int>(stk1.size());
            while (j && nums[i] > nums[stk1[j - 1]]) {
                --j;
            }
            stk2.insert(stk2.end(), stk1.begin() + j, stk1.end());
            stk1.erase(stk1.begin() + j, stk1.end());
            stk1.push_back(i);
        }
        return res;
    }
...
#ifdef LOCAL
int main() {
    auto nums = vector({1, 2, 3, 4, 3, 2, 1, 2, 3, 4});
    Solution s;
    dbg(s.secondGreaterElement(nums));
    return 0;
}
#endif
