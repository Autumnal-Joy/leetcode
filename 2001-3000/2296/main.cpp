...
public:
    ...
    void addText(const string &text) {
        pre.insert(pre.end(), text.begin(), text.end());
    }

    int deleteText(const int k) {
        const auto m = static_cast<int>(pre.size());
        const auto cnt = std::min(m, k);
        pre.erase(pre.end() - cnt, pre.end());
        return cnt;
    }
    ...
    string cursorLeft(const int k) {
        const auto m = static_cast<int>(pre.size());
        const auto cnt = std::min(m, k);
        suf.insert(suf.begin(), pre.end() - cnt, pre.end());
        pre.erase(pre.end() - cnt, pre.end());
        return m - cnt > 10 ? string(pre.end() - 10, pre.end()) : string(pre.begin(), pre.end());
    }

    string cursorRight(const int k) {
        const auto m = static_cast<int>(pre.size());
        const auto n = static_cast<int>(suf.size());
        const auto cnt = std::min(n, k);
        pre.insert(pre.end(), suf.begin(), suf.begin() + cnt);
        suf.erase(suf.begin(), suf.begin() + cnt);
        return m + cnt > 10 ? string(pre.end() - 10, pre.end()) : string(pre.begin(), pre.end());
    }
};
...
#ifdef LOCAL
int main() {
    const auto te = new TextEditor();// 当前 text 为 "|" 。（'|' 字符表示光标）
    te->addText("leetcode");         // 当前文本为 "leetcode|" 。
    dbg(te->deleteText(4));          // 返回 4
    // 当前文本为 "leet|" 。
    // 删除了 4 个字符。
    te->addText("practice");// 当前文本为 "leetpractice|" 。
    dbg(te->cursorRight(3));// 返回 "etpractice"
    // 当前文本为 "leetpractice|".
    // 光标无法移动到文本以外，所以无法移动。
    // "etpractice" 是光标左边的 10 个字符。
    dbg(te->cursorLeft(8));// 返回 "leet"
    // 当前文本为 "leet|practice" 。
    // "leet" 是光标左边的 min(10, 4) = 4 个字符。
    dbg(te->deleteText(10));// 返回 4
    // 当前文本为 "|practice" 。
    // 只有 4 个字符被删除了。
    dbg(te->cursorLeft(2));// 返回 ""
    // 当前文本为 "|practice" 。
    // 光标无法移动到文本以外，所以无法移动。
    // "" 是光标左边的 min(10, 0) = 0 个字符。
    dbg(te->cursorRight(6));
    delete te;
}
#endif
