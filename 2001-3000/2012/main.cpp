...
class Solution {
public:
    int sumOfBeauties(const vector<int> &nums) {
        const auto n = static_cast<int>(nums.size());
        auto preMax = vector(n, MIN_I);
        auto sufMin = vector(n, MAX_I);
        for (int i = 1; i < n; ++i) {
            preMax[i] = std::max(preMax[i - 1], nums[i - 1]);
        }
        for (int i = n - 2; i >= 0; --i) {
            sufMin[i] = std::min(sufMin[i + 1], nums[i + 1]);
        }
        auto res = 0;
        for (int i = 1; i < n - 1; ++i) {
            if (nums[i] > preMax[i] && nums[i] < sufMin[i]) {
                res += 2;
