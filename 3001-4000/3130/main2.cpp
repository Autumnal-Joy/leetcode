...
auto memo = Array<int, 1001, 1001, 2>();
...
public:
    int numberOfStableArrays(const int zero, const int one, const int limit) {
        ...
        auto dfs = [limit](auto &&self, const int zero, const int one, const int cur) -> int {
            if (zero < 0 || one < 0) return 0;
            if (zero == 0) {
                return cur == 1 && one <= limit;
            }
            if (one == 0) {
                return cur == 0 && zero <= limit;
            }

            auto &m = memo[zero][one][cur];
            if (m != -1) return m;

            auto res = 0ll;
            if (cur == 0) {
                res = 0ll + self(self, zero - 1, one, 0) +
                      self(self, zero - 1, one, 1) +
                      MOD - self(self, zero - limit - 1, one, 1);
            } else {
                res = 0ll + self(self, zero, one - 1, 0) +
                      self(self, zero, one - 1, 1) +
                      MOD - self(self, zero, one - limit - 1, 0);
            }
...
int main() {
    Solution s;
    dbg(s.numberOfStableArrays(1, 1, 2));
    return 0;
}
#endif
