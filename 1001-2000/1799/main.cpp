...
public:
    int maxScore(const vector<int> &nums) {
        ...
        auto gcd = Array<int, 15, 15>();
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                gcd[i][j] = gcd[j][i] = std::gcd(nums[i], nums[j]);
            }
        }
        auto dp = vector(1 << n, 0);
        for (int s1 = 0; s1 < 1 << n; ++s1) {
            const auto cnt = popcount(static_cast<unsigned>(s1));
            if (cnt & 1) continue;
            for (int i = 0; i < n; ++i) {
                if (s1 & (1 << i)) continue;
                for (int j = i + 1; j < n; ++j) {
                    if (s1 & (1 << j)) continue;
                    const auto s2 = s1 | (1 << i) | (1 << j);
                    dp[s2] = max(dp[s2], dp[s1] + (cnt / 2 + 1) * gcd[i][j]);
                }
            }
        }
        return dp.back();
