...
public:
    ...
    void visit(const string &url) {
        history.resize(cnt + 1);
        history[cnt++] = url;
    }

    string back(const int steps) {
        cnt = std::max(1, cnt - steps);
        return history[cnt - 1];
    }

    string forward(const int steps) {
        cnt = std::min(static_cast<int>(history.size()), cnt + steps);
        return history[cnt - 1];
    }
};
...
#ifdef LOCAL
int main() {
    const auto bh = new BrowserHistory("leetcode.com");
    bh->visit("google.com");
