...
class Solution {
public:
    int divide(int dividend, const int divisor) {
        if (dividend == numeric_limits<int>::max() && divisor == 1) return numeric_limits<int>::max();
        if (dividend == numeric_limits<int>::max() && divisor == -1) return -numeric_limits<int>::max();
        ...
        while (dividend <= divisor) {
            auto temp = divisor, coe = 1;
            while (temp > numeric_limits<int>::min() >> 1 && temp + temp > dividend) {
                temp <<= 1;
                coe <<= 1;
            }
            dividend -= temp;
