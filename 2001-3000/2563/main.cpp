...
public:
    long long countFairPairs(vector<int> &nums, int lower, int upper) {
        ...
        auto res = 0ll;
        for (int i = 0; i < n; ++i) {
            const int begin = first(i, n, [&](int j) {
                return nums[i] + nums[j] >= lower;
            });
            const int end = last(i, n, [&](int j) {
                return nums[i] + nums[j] <= upper;
            });
            if (end - begin >= 0)
                res += end - begin + 1;
        }
...
#ifdef LOCAL
int main() {
    auto v = vector({1, 2, 5, 7, 9});
    Solution s;
    s.countFairPairs(v, 11, 11);
    return 0;
}
#endif
