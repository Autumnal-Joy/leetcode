...
public:
    int numberOfArithmeticSlices(const vector<int> &nums) {
        ...
        auto hash = unordered_map<long long, vector<int>>();
        for (int i = 0; i < n; i++) {
            hash[nums[i]].push_back(i);
        }
        auto dp = Array<int, 1001, 1001>();
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < i; ++j) {
                const auto diff = 0ll + nums[i] - nums[j];
                const auto prev = nums[j] - diff;
                if (!hash.contains(prev)) continue;
                for (const auto idx: hash[prev]) {
                    if (idx >= j) continue;
                    dp[i][j] += dp[j][idx] + 1;
                }
                res += dp[i][j];
