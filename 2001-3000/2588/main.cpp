...
class Solution {
    static int lb(const int n) { return n & -n; }

public:
    long long beautifulSubarrays(const vector<int> &nums) {
        const auto n = static_cast<int>(nums.size());
        auto mp = unordered_map<int, long long>();
        int d = 0;
        mp[d] = 1;
        long long res = 0;
        for (int i = 0; i < n; ++i) {
            for (int j = nums[i]; j; j -= lb(j)) {
                d ^= lb(j);
            }
            res += mp[d]++;
        }
