...
public:
    int maxDistance(const vector<vector<int>> &arrays) {
        auto pq = priority_queue<PII>();
        auto pqr = priority_queue_rev<PII>();
        for (int i = 0; i < arrays.size(); ++i) {
            pq.emplace(arrays[i].back(), i);
            pqr.emplace(arrays[i].front(), i);
        }
        auto [maxVal1, maxIdx1] = pq.top();
        pq.pop();
        auto [maxVal2, maxIdx2] = pq.top();
        pq.pop();
        auto [minVal1, minIdx1] = pqr.top();
        pqr.pop();
        auto [minVal2, minIdx2] = pqr.top();
        pqr.pop();

        if (maxIdx1 != minIdx1) {
            return maxVal1 - minVal1;
        }
        return std::max(maxVal1 - minVal2, maxVal2 - minVal1);
    }
};

#ifdef LOCAL
int main() {
    auto arrays = vector<vector<int>>({{1, 2, 3}, {4, 5}, {1, 2, 3}});
