...
class Solution {
public:
    vector<double> getCollisionTimes(const vector<vector<int>> &cars) {
        const auto n = static_cast<int>(cars.size());
        auto res = vector(n, -1.0);
        auto stk = stack<int>();
        for (int i = n - 1; i >= 0; --i) {
            ...
            while (!stk.empty()) {
                const auto next = stk.top();
                const auto time = 1.0 * (cars[next][0] - cars[i][0]) / (cars[i][1] - cars[next][1]);
                if (res[next] < 0 || time <= res[next]) {
                    res[i] = time;
                    break;
                }
                stk.pop();
