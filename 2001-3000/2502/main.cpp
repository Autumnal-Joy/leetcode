...
class Allocator {
    int n;
    vector<bool> mem;
    unordered_map<int, vector<PII>> used;

public:
    explicit Allocator(const int n) : n(n), mem(n) {}

    int allocate(const int size, const int mID) {
        int l = 0, r = 0;
        auto cnt = 0;
        while (r < n) {
            if (!mem[r]) {
    ...
    int freeMemory(const int mID) {
        int cnt = 0;
        for (const auto &[idx, size]: used[mID]) {
...
int main() {
    ...
    dbg(obj->allocate(1, 1));
    dbg(obj->allocate(1, 2));
    dbg(obj->allocate(1, 3));
    dbg(obj->freeMemory(2));
    dbg(obj->allocate(3, 4));
    dbg(obj->allocate(1, 1));
    dbg(obj->allocate(1, 1));
    dbg(obj->freeMemory(1));
    dbg(obj->allocate(10, 2));
    dbg(obj->freeMemory(7));
    delete obj;
    return 0;
}
#endif
